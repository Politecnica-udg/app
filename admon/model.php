<?php
	class general_A extends datebase{
		...
		public function evaluados($cod){
			global $conf_poli;
			$query = $this->consulta("SELECT distinct carr_cal, gdo_cal, tno_cal, gpo_cal, evaluar, COUNT(*) AS cont FROM cali
				INNER JOIN asig_prof on ccar_prof=carr_cal AND gdo_prof = gdo_cal AND tur_prof=tno_cal AND gpo_prof=gpo_cal
				WHERE cic_cal='$conf_poli[cal_prof]' AND cod_prof = '$cod'
				GROUP BY carr_cal, gdo_cal, tno_cal, gpo_cal, evaluar");
			if($this->numero_de_filas($query) > 0){
				while ( $tsArray = $this->fetch_assoc($query) ) {
					$data[] = $tsArray;
				}
