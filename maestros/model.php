	...
	class general_M extends datebase{
		...
		public function materias(){
			...
			$query = $this->consulta("SELECT distinct ccar_prof,abr_car,gdo_prof,tur_prof,gpo_prof,casg_prof,nom_pes,cal_prof,cod_prof,capint_prof, cin FROM `asig_prof`
				...
				WHERE cal_prof='$conf_poli[cal_prof]' and cod_prof='$_SESSION[codigo]';");
		...
		public function datos(){
			global $conf_poli;
			$query = $this->consulta(" SELECT DISTINCT cod_ev,nom_ubi,cal_int_ev,tfal_int_ev, tclas_int_ev  FROM evaluacion INNER JOIN ubicacion ON calu_ubi=cod_ev WHERE ciclo = '$conf_poli[cal_prof]' and car_ev = '$_GET[c]' and gdo_ev = '$_GET[g]' and  gpo_ev = '$_GET[gr]' and asig_ev = '$_GET[m]' ORDER BY nom_ubi ");
			if($this->numero_de_filas($query) > 0){
				while ( $tsArray = $this->fetch_assoc($query) ) {
					$data[] = $tsArray;			
				}
